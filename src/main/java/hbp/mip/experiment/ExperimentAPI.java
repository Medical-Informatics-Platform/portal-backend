...
public class ExperimentAPI {
    ...
    public ResponseEntity<ExperimentsDTO> getExperiments(Authentication authentication,
            ...
            @RequestParam(name = "notMine", required = false, defaultValue = "false") boolean notMine,
    ...
    public ResponseEntity<ExperimentDTO> getExperiment(Authentication authentication,
            @PathVariable("uuid") String uuid) {
        var logger = new Logger(activeUserService.getActiveUser(authentication).username(),
    ...
    @PostMapping
    public ResponseEntity<ExperimentDTO> createExperiment(Authentication authentication,
        ...
        logger.info("Request for experiment creation. RequestBody: "
                + JsonConverters.convertObjectToJsonString(experimentExecutionDTO));
    ...
    public ResponseEntity<ExperimentDTO> updateExperiment(Authentication authentication,
            @RequestBody ExperimentDTO experimentRequest, @PathVariable("uuid") String uuid) {
        var user = activeUserService.getActiveUser(authentication);
        var logger = new Logger(user.username(), "(PATCH) /experiments/" + uuid);
        logger.info("Request for experiment update with id: " + uuid + ".  Request Body: "
                + JsonConverters.convertObjectToJsonString(experimentRequest));
    ...
    public ResponseEntity<ExperimentDTO> createTransientExperiment(Authentication authentication,
            @RequestBody ExperimentExecutionDTO experimentExecutionDTO) {
        var logger = new Logger(activeUserService.getActiveUser(authentication).username(),
                "(POST) /experiments/transient");
        logger.info("Request for transient experiment creation. RequestBody: "
                + JsonConverters.convertObjectToJsonString(experimentExecutionDTO));
